...
- **IMPORTANT**: Variables declared inside a function or method are visible throughout the **entire** function/method
- **NO BLOCK-LEVEL SCOPE**: Variables declared within `If`, `While`, `Repeat`, `For`, `For each`, or `Case of` blocks remain accessible outside those blocks
- **AVOID** assuming block-level scope like in JavaScript, C, or other modern languages

```4d
// ⚠️ IMPORTANT - Variable scope example
#DECLARE($condition: Boolean)

If ($condition)
    var $message: Text:="Condition is true"
End if

// $message is still accessible here, even outside the If block
// This is different from many other languages!
ALERT($message)  // Will work if $condition was True, undefined behavior if False

// ✅ BETTER - Declare at function level
var $message: Text
If ($condition)
